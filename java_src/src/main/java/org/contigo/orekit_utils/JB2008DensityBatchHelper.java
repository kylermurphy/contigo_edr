...
public class JB2008DensityBatchHelper {
    ...
    /**
     * Compute JB2008 atmospheric density at each point along a satellite track.
     *
    ...
    public static double[] getDensity(
        ...
        JB2008 atmosphere = new JB2008(params, SUN, EARTH);

        double[] density = new double[n];
        for (int i = 0; i < n; i++) {
            AbsoluteDate date = referenceDate.shiftedBy(offsetSeconds[i]);
            Vector3D pos = new Vector3D(positions[i][0], positions[i][1], positions[i][2]);
    ...
    /**
     * Compute JB2008 atmospheric density using space weather data bundled with
     * the Orekit data archive loaded at JVM startup.
     *
     * This is the simpler alternative to {@link #getDensity}: no space weather
     * arrays need to be supplied from Python. Orekit reads the SOLFSMY (solar
     * flux) and DTCFILE (exospheric temperature correction) files from the data
     * context configured by {@code setup_orekit_data}.
     *
